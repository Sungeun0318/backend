...
public class Exam1 {
    public static void main(String[] args) {
        ...
        // [3] 목적 ; 가변(변하는요소들의 수) 길이의 배열 제공과 기능/메소드 제공
        // [4] 사용법
            // 1. 선언/정의 하기 : ArrayList< 항목타입> 변수명 = new ArrayList()<>;
            // -> < > 괄호는 제네릭 타입으로 리스트 안에 저장 할 항목들의 타입
            // -> 제네릭타입은 기본타입 불가능하므로 int -> Inteager클래스 변경해서 작성 (8개 타입)
        ArrayList<String> 리스트변수명 = new ArrayList<>(); // 여러 개 String객체를 저장하는 리스트
        // vs String[] 변수명 = new String[100];
            // 2. 요소 추가 : .add(새로운 값); 마지막 빈 요소에 요소 추가
        리스트변수명.add("유재석");
            // 3. 요소들 출력 :
