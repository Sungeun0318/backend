...
public class Practice17 {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < draw.length-1; i++) { // 배열 순회
            lottoSet.add(draw[i]); // index번째 번호를 set 컬렉션 객체 대입
        }
        ...
        map.put("유재석", 95); // key 와 value으로 entry한쌍 구조 추가
        map.put("강호동", 100); // key 와 value으로 entry한쌍 구조 추가
        map.put("신동엽", 78); // key 와 value으로 entry한쌍 구조 추가
        map.put("유재석", 67); // key 와 value으로 entry한쌍 구조 추가
        map.put("서장훈", 100); // key 와 value으로 entry한쌍 구조 추가
        System.out.println("map.get(\"유재석\") = " + map.get("유재석"));
        System.out.println("map.size() = " + map.size()); // 모든 앤트리(key:value) 총개수 호출
        ...
        for(String key : map.keySet()) { // 방법1]
            System.out.println("key + \":\" + map.get(key) = " + key + ":" + map.get(key));
        }
        map.keySet().forEach((key) -> {// 방법2] 변수명.forEach((반복변수) -> {실행문;});
            System.out.println("key + \":\" + map.get(key) = " + key + ":" + map.get(key));
        });

        // [7] 자바 외 다른 곳에서 데이터가 오는 경우 정리 - 데이터 전처리( 현재 상황에 맞게 데이터 수정)
        Map<String, String> jsonMap = new HashMap<>();
        String json = "{\"name\":\"유재석\",\"age\":45,\"city\":\"서울\"}";
        json = json.replace( "{",""); // {} 제거, 문자열.replace("기존문자", "새로운문자");
        System.out.println("json = " + json); // 확인

        String[] jsonAry = json.split(","); // 문자열.split("기존문자") : 문자열내 "기존문자" 기준으로 분리하여 String[]반환
        System.out.println(Arrays.toString(jsonAry));
        for(int i = 0; i < jsonAry.length-1; i++){ // 분리된 조각들을 하나씩 확인
            String str = jsonAry[i]; // System.out.println("str = " + str);
            String[] strAry = str.split(":");// : 기준 split(2개로만)
            // 예] strAry[0] = "name", strAry[1] = "유재석", 즉] 0번 인덱스는 key(속성), 1번인덱스 value(값)
        ...
        jsonMap.keySet().forEach((key) -> {
            System.out.println("key + \":\" + jsonmap.get(key) = " + key + ":" + jsonMap.get(key));
        });
