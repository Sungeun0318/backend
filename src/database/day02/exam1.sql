...
create table test3(
    ...
    날짜속성1 date, 시간속성2 time, 날짜시간속성3 datetime,
...
create table test4(
	...
	-- 속성명5 mediumint primary key   -- primary key : pk(식별/기본)키, 식별가능한 고유한 값 가진 키
								  -- 중복없음 unique + 빈칸없음not null, 학번, 사번, 주민등록번호, 제품번호 등등
								  -- vs constraint primary key (속성명)
);

create table test5(
	속성명1 bigint,
	constraint foreign key(속성명1) references 테이블명 (속성명4) on delete restrict
    -- foreign key : fk(참조/외래)키, 다른 테이블의 기본키(pk)참조하는 키
	-- constraint foreign key(FK속성명) references 테이블명 (pk필드명) 
    -- 중복가능, 빈칸가능 
	-- pk(1) : fk(N) 관계에서 수강신청학번, 휴가신청학번사번, 구매한제품번호 등등
    -- on update/delete restrict : fk와 pk 참조 관계일때 pk 삭제/수정 불가능 관계 < 자식이 있으면 부모 삭제 불가능 >, 기본값
					-- cascade : pk가 삭제/수정 될 때 fk도 같이 삭제 관계 < 게시물삭제되면 댓글도 같이 삭 >
					-- set null : pk가 삭제/수정 될 때 fk는 null 변경 관계, < 게시물삭제되면 댓글 참조는 null >
    );
    select * from test4;
    select * from test5;
