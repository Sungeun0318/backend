...
public class Exam1 {
    public static void main(String[] args) {
        ...
        // [1] type = 자료형 = 분류  = 자료들을 분류/나누는 방법, 다형성 특징( 다양한 형/모양 갖는다) -- 꼭 정리해놓기
        // (1) 자동 타입 변환 ( 묵시적 : 개발자 따로 할 할게 없다.) , 손실 없다.
        // byte -> short -> int -> long -> float -> double (작은 상자 -> 큰상자)
        byte byteValue = 10;  // 10 자료를 byte 타입으로 분류 했다.
        short shortValue = byteValue;    // byte 타입 -> short 타입 변환            // 자료는 10이지만
        ...
        double dvalue = 3.14;
        float fvalue = (float) dvalue; // (새로운 타입) 기존값
        long lvalue = (long) fvalue;
        int ivalue = (int) lvalue;
        short svalue = (short) ivalue;
        byte bvalue = (byte) svalue;    // bvalue에는 3자료가 들어있다. 손실 발생.
